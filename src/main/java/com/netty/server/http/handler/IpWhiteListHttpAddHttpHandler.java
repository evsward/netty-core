...
@URIBasedPolicy("/admin/ipWhiteList/httpAdd")
public class IpWhiteListHttpAddHttpHandler implements HttpMessageHandler<HttpRequestMessage,HttpResponseMessage> {
 
	protected static Logger logger = LoggerFactory.getLogger(IpWhiteListHttpAddHttpHandler.class);
	
	private static final String RETURN_MESSAGE = "{\"result\":\"success\",\"message\":\"%s\"}";
	private static final String FAILED_MESSAGE = "{\"result\":\"error\",\"message\":\"need params key and ip . Usage: http://ip:port/admin/ipWhiteList/tcpAdd?key=http.uri.global&ip=172.28.19.76\"}";
